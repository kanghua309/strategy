...
def sector_filter2(tradeable_count, sector_exposure_limit):
        """
        Mask for Pipeline in create_tradeable. Limits each sector so as not to be over-exposed
        ...
        transport_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                   mask=sector.eq(g_inds['交通运输'.decode("UTF-8")]))  #
        instrument_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                    mask=sector.eq(g_inds['仪器仪表'.decode("UTF-8")]))
        media_entertainment_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['传媒娱乐'.decode("UTF-8")]))  #
        water_supply_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                      mask=sector.eq(g_inds['供水供气'.decode("UTF-8")]))  #
        highway_bridge_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['公路桥梁'.decode("UTF-8")]))  #
        other_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                               mask=sector.eq(g_inds['其它行业'.decode("UTF-8")]))
        animal_husbandry_fishery_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['农林牧渔'.decode("UTF-8")]))  #
        pesticide_fertilizer_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['农药化肥'.decode("UTF-8")]))
        chemical_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                  mask=sector.eq(g_inds['化工行业'.decode("UTF-8")]))  #
        chemical_fiber_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                        mask=sector.eq(g_inds['化纤行业'.decode("UTF-8")]))
        medical_device_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                        mask=sector.eq(g_inds['医疗器械'.decode("UTF-8")]))
        printing_packaging_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['印刷包装'.decode("UTF-8")]))
        power_plant_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                     mask=sector.eq(g_inds['发电设备'.decode("UTF-8")]))
        business_department_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['商业百货'.decode("UTF-8")]))  #
        plastics_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                  mask=sector.eq(g_inds['塑料制品'.decode("UTF-8")]))
        furniture_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                   mask=sector.eq(g_inds['家具行业'.decode("UTF-8")]))  #
        appliance_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                   mask=sector.eq(g_inds['家电行业'.decode("UTF-8")]))  #
        building_materials_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['建筑建材'.decode("UTF-8")]))  #
        development_zone_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                          mask=sector.eq(g_inds['开发区'.decode("UTF-8")]))
        real_estate_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                     mask=sector.eq(g_inds['房地产'.decode("UTF-8")]))  #
        motorcycle_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                    mask=sector.eq(g_inds['摩托车'.decode("UTF-8")]))
        nonferrous_metals_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['有色金属'.decode("UTF-8")]))  #
        clothing_footwear_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['服装鞋类'.decode("UTF-8")]))  #
        machinery_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                   mask=sector.eq(g_inds['机械行业'.decode("UTF-8")]))  #
        time_shares_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                     mask=sector.eq(g_inds['次新股'.decode("UTF-8")]))
        cement_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                mask=sector.eq(g_inds['水泥行业'.decode("UTF-8")]))
        automobile_manufacturing_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['汽车制造'.decode("UTF-8")]))  #
        coal_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                              mask=sector.eq(g_inds['煤炭行业'.decode("UTF-8")]))  #
        material_trade_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                        mask=sector.eq(g_inds['物资外贸'.decode("UTF-8")]))
        environmental_protection_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['环保行业'.decode("UTF-8")]))  #
        glass_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                               mask=sector.eq(g_inds['玻璃行业'.decode("UTF-8")]))
        biopharmaceuticals_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['生物制药'.decode("UTF-8")]))  #
        power_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                               mask=sector.eq(g_inds['电力行业'.decode("UTF-8")]))
        electronic_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                    mask=sector.eq(g_inds['电器行业'.decode("UTF-8")]))
        electronic_information_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['电子信息'.decode("UTF-8")]))  #
        electronic_device_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['电子器件'.decode("UTF-8")]))
        oil_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(g_inds['石油行业'.decode("UTF-8")]))
        textile_machinery_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['纺织机械'.decode("UTF-8")]))
        textile_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                 mask=sector.eq(g_inds['纺织行业'.decode("UTF-8")]))
        integrated_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                    mask=sector.eq(g_inds['综合行业'.decode("UTF-8")]))
        ship_manufacturing_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['船舶制造'.decode("UTF-8")]))
        paper_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                               mask=sector.eq(g_inds['造纸行业'.decode("UTF-8")]))
        hotel_tour_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                    mask=sector.eq(g_inds['酒店旅游'.decode("UTF-8")]))  #
        wine_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(g_inds['酿酒行业'.decode("UTF-8")]))
        financial_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                   mask=sector.eq(g_inds['金融行业'.decode("UTF-8")]))  #
        steel_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                               mask=sector.eq(g_inds['钢铁行业'.decode("UTF-8")]))  #
        ceramics_trim = AverageDollarVolume(window_length=21).top(threshold,
                                                                  mask=sector.eq(g_inds['陶瓷行业'.decode("UTF-8")]))
        aircraft_manufacturing_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['飞机制造'.decode("UTF-8")]))
        food_industry_trim = AverageDollarVolume(window_length=21).top(threshold, mask=sector.eq(
            g_inds['食品行业'.decode("UTF-8")]))  #

        return ceramics_trim | paper_trim | ship_manufacturing_trim | integrated_trim | textile_machinery_trim | electronic_trim | glass_trim | material_trade_trim
