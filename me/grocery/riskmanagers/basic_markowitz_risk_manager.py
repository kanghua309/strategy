...
class BasicMarkowitzRiskManager(RiskManager):
    ...
    def optimalize(self,candidates,factors):
        ...
        returns = candidates[factors['RETURNS']]
        cov_mat = returns.cov()
        ...
        constraints.extend([w >= MIN_LONG_POSITION_SIZE, w <= MAX_LONG_POSITION_SIZE])  # long exposure
             # risk
        print "MIN_SHORT_POSITION_SIZE %s, MAX_SHORT_POSITION_SIZE %s,MAX_BETA_EXPOSURE %s" % (MIN_LONG_POSITION_SIZE, MAX_LONG_POSITION_SIZE)
        # 版块对冲当前，因为股票组合小，不合适
        sector_dist = {}
        idx = 0
